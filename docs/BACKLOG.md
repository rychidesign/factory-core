...
- **Tailscale hostname** — `install-tailscale.sh` defaultuje `--hostname=factory-server`, ale operátorův homelab může už být v tailnetu pod jiným jménem (skynetdev v reálu). Skript skipne `tailscale up` pokud authenticated, ale doc by měl říct "or whatever your existing tailscale hostname is".
- **Cloudflare Access setup vyžaduje admin API token** — runtime `CLOUDFLARE_API_TOKEN` (Pages perms) nestačí na `accounts/{id}/access/*` endpoints. Bootstrap proces by měl umět buď (a) automaticky vygenerovat dočasný admin token přes UI flow, nebo (b) jasně popsat dvě tokens — runtime + admin — v `config/secrets.env.example`. Současně dokumentováno v server-setup.md jako manuální dashboard step, což operátora občas nudilo. ADR worth?
- **Zero Trust onboarding edge case** — během Wave 1 step 1.7 v reálu se Zero Trust org vytvořila automaticky při `cloudflared tunnel login` (auth_domain `<account>.cloudflareaccess.com`). To není standard Cloudflare flow — někdy vyžaduje explicit onboarding přes UI. Doc by měl popsat oba scénáře.
- **`config/secrets.env.example`** chybí `CLOUDFLARE_ACCESS_ADMIN_TOKEN` — admin token použitý pro Wave 1 setup Access policy. Buď doplnit jako optional one-shot key, nebo dokumentovat jako bootstrap-only (nepoužívá se runtime).
- **Real-server learnings z Wave 1.7** — provisioning na skynetdev (Ubuntu 24.04) odhalil 3 drobné bugy v skriptech / doc, opraveny v fix-up commitu po doladění. Pro budoucí wavy: **vždy** otestuj scripts + doc na čerstvém serveru / VM před označením kroku za hotový.
